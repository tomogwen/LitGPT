...
class Head(nn.Module):
    ...
    def __init__(self, head_size):
        ...
        self.register_buffer('tril', torch.tril(torch.ones(BLOCK_SIZE, BLOCK_SIZE)))  # non-trainable param
    ...
    def forward(self, x):
        ...
        wei = wei.masked_fill(self.tril[:T, :T]==0, float('-inf'))  # making it a decoder block by masking 'forward' elements
...
class TransformerDecoder(nn.Module):
    def __init__(self):
        ...
        self.blocks = nn.Sequential(*[Block(N_EMBD, N_HEADS) for _ in range(NUM_BLOCKS)])
